...
def getBestGrayValue(image, mediaGrayValue):
    ...
    for row in range(height):
        for col in range(width):
            grayValuePart_1.append(image[row][col]) if image[row][col] > mediaGrayValue else grayValuePart_2.append(image[row][col])
...
if __name__ == '__main__':
    ...
    # 获取最优阈值，并输出
    bestGrayValue = getBestGrayValue(srcImg, mediaGrayValue)
    print(bestGrayValue)
    # 按最优阈值分割
